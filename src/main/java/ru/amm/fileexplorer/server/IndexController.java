...
public class IndexController {
    ...
    public ModelAndView searchResults(@RequestParam(name = "path") String path, @RequestParam(name = "search") String pattern) {
        ...
        dirContents = new DirectoriesFilter( ).filter( pattern, dirContents );
        Map<String, Object> data = new HashMap<>( );
        data.put( "directory", dirContents );
        return new ModelAndView( "index", data );
    ...
    public String singleFileUpload(@RequestParam("file") MultipartFile file,
        ...
        Path path = publishPath.resolve( pathToUpload );
        UploadService uploadService = new FileSystemUploadService( path );
        uploadService.upload( file );
        System.out.println( path );


        redirectAttributes.addAttribute( "path", pathToUpload );
        redirectAttributes.addAttribute( "sort", "DEFAULT" );
    ...
    public void getFile(
        ...
        try {
            InputStream is = new FileInputStream( publishPath.resolve( fileName ).toString( ) );
